...
class AudioTcpServer {
    ...
    private lazy var serverBootstrap = ServerBootstrap(group: group)
        ...
        .serverChannelOption(ChannelOptions.backlog, value: 256) // max clients
        .serverChannelOption(ChannelOptions.socketOption(.so_reuseaddr), value: 1)
        
        // Set the handlers that are applied to the accepted Channels
        .childChannelInitializer { channel in
            // Add handler that will buffer data until a \n is received
            channel.pipeline.addHandlers([BackPressureHandler(), H16D320Ch1Handler()])
        }
...
final class H16D320Ch1Handler: ChannelInboundHandler {
